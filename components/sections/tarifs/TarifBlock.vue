  ...
  <div class="tarif mt-4 lg:mt-16">
   ...
   <div class="tarif__subscribers flex-wrap justify-center mt-6 lg:mt-8">
      <div
        ...
        :class="{
          'tarif__subscriber-item_active': tarif.subscribers === activeTarif.subscribers,
        }"
        class="tarif__subscriber-item text-center"
        @click="activeTarif = tarif"
      >
        <span class="text text-white"> {{ numberFormat(tarif.subscribers) }} </span>
      </div>
    </div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-12 mt-12 lg:mt-16">
      <div class="price-card">
        ...
        <div class="flex flex-col items-center justify-center h-full py-4 lg:p-0">
          <div class="text flex items-center">
            <span class="color-green text-xl lg:text-4xl font-semibold"> {{ activeTarif.yPrice.month }} ₽</span>
            <span class="text-xs lg:text-xl ml-2">/ месяц</span>
          </div>
          <span class="mt-2 text text-md lg:text-xl">{{ activeTarif.yPrice.value }} ₽ / год</span>
      ...
      <div class="price-card">
        ...
        <div class="flex flex-col items-center justify-center h-full py-4 lg:p-0">
          <div class="text flex items-center">
            <span class="color-green text-xl lg:text-4xl font-semibold">{{ activeTarif.sPrice.month }} ₽</span>
            <span class="text-xs lg:text-xl ml-2">/ месяц</span>
          </div>
          <span class="mt-2 text text-md lg:text-xl">{{ activeTarif.sPrice.value }} ₽ / год</span>
      ...
      <div class="price-card">
        ...
        <div class="flex flex-col items-center justify-center h-full p-4 lg:p-0">
          <div class="text flex items-center">
            <span class="color-green text-xl lg:text-4xl font-semibold">{{ activeTarif.qPrice.month }} ₽</span>
            <span class="text-xs lg:text-xl ml-2">/ месяц</span>
          </div>
          <span class="mt-2 text text-md lg:text-xl">{{ activeTarif.qPrice.value }} ₽ / год</span>
      ...
      <div class="price-card">
        ...
        <div class="flex flex-col items-center justify-center h-full py-4 lg:p-0">
          <div class="text flex items-center">
            <span class="color-green text-xl lg:text-4xl font-semibold">{{ activeTarif.mPrice.month }} ₽</span>
            <span class="text-xs lg:text-xl ml-1">/ месяц</span>
          </div>

          <Button text="Подключиться" class="px-2 py-2 mt-12 lg:px-8" />
        </div>
      </div>
    </div>
  </div>
...
const tarifs = [
  ...
  { 
    subscribers: 100000, 
    mPrice: { month: 30000 }, 
    qPrice: { month: 19190, value: 57570 },
    sPrice: { month: 17170, value: 103020 },
    yPrice: { month: 14140, value: 169680 },
  },
  { 
    subscribers: '150 000+', 
    mPrice: { month: 'от 40000' }, 
